...
🎨 Генерация изображений из текстовых подсказок в режиме <b>🎨 Художник</b> /mode
...
async def _vision_message_handle_fn(
    ...
    if use_new_dialog_timeout:
        if (datetime.now() - db.get_user_attribute(user_id, "last_interaction")).seconds > config.new_dialog_timeout and len(db.get_dialog_messages(user_id)) > 0:
            db.start_new_dialog(user_id)
            await update.message.reply_text(f"Начинаю новый диалог из-за тайм-аута (<b>{config.chat_modes[chat_mode]['name']}</b> режим) ✅", parse_mode=ParseMode.HTML)
    db.set_user_attribute(user_id, "last_interaction", datetime.now())
...
async def unsupport_message_handle(update: Update, context: CallbackContext, message=None):
    error_text = f"Я не умею читать файлы или видео. Отправьте изображение в обычном режиме (Быстрый режим)."
    logger.error(error_text)
    await update.message.reply_text(error_text)
    return
...
def get_chat_mode_menu(page_index: int):
    ...
    text = f"Выберите <b>режим чата</b> ({len(config.chat_modes)} доступных режима):"
